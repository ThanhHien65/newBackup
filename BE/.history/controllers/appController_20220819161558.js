        ...
        if (req.body.password.length != 0) {
          bcrypt.hash(req.body.password, 10, (err, hash) => {
            if (err) {
              ...
              sql.CreateAccount(req.body.username, hash, (result) => {
                if (result.code == undefined) {
                  res.status(201).send({
...
exports.LoginAccount = (req, res) => {
  sql.Accountexist(req.body.username, (checkAccount) => {
    if (checkAccount.length) {
      bcrypt.compare(
        req.body.password,
        ...
        (berr, bresult) => {
          if (bresult === true) {
            ...
            sql.LastLogin(checkAccount[0].id);
            res.status(200).send({
              msg: "Logged in!",
              token,
            });
...
exports.GetAccount = (req, res) => {
  ...
  sql.getAccount(decoded.id, (result) => {
    res.status(200).send({
      decoded,
    });
  });
