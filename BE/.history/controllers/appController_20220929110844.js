          ...
          if (bresult === true) {
            ...
            Backup.statusBackupday((backupday, err) => {
              Backup.statusASPBackupday((aspbackup, err) => {
                Backup.dataImage((allimg, err) => {
                  res.status(200).send({
                    msg: "Logged in!",
                    token,
                    backupday,
                    aspbackup,
                    ofmonitor: allimg["ofmonitor"],
                    ofgw: allimg["ofmonitor"],
                  });
                });
              });
            });
        ...
        Backup.statusBackupday((backupday, err) => {
          Backup.statusASPBackupday((aspbackup, err) => {
            ...
            fs.readdir(directoryPath, (err, files) => {
              ...
              res.status(200).send({ backupday, aspbackup, img });
            });
          });
        });
        console.log(
...
exports.ImageMonitor = (req, res) => {
  ...
  console.log(
    parseIp(req),
    `${moment().format("YYYY-MM-ddd HH:mm:ss")} ||`,
    `[GET-STATUS-IMAGE-${req.params.pic}]`
  );
};
exports.dataimg = (req, res) => {
  Backup.dataImage((result, err) => {
    console.log(
      parseIp(req),
      `${moment().format("YYYY-MM-ddd HH:mm:ss")} ||`,
      `[GET-ALLIMG]`
