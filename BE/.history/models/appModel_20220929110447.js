...
Backup.Accountexist = (username, checkaccount) => {
  sql.query(
...
Backup.CreateAccount = (username, password, result) => {
  sql.query(
...
Backup.dataImage = (result) => {
  const ofmonitor = "SELECT * FROM `ofmonitor`";
  const ofgw = "SELECT * FROM `ofgw`";
  sql.query(ofmonitor, (err, ofmonitor) => {
    sql.query(ofgw, (err, ofgw) => {
      result({ ofmonitor, ofgw });
    });
  });
};
