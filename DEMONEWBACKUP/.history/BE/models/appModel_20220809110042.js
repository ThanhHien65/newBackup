...
Backup.changeStatus = (status, result) => {
  sql.query(`update filebackup set check_value=0 where id = (select k.id from (select max(id) as id from filebackup where ifno='1092' and DATE(date)=current_date) k)
