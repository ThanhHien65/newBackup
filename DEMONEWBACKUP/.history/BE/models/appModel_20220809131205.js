...
Backup.changeStatus = (status, result) => {
  sql.query(
    `update bigdata set check_value=1 where id = (select k.id from (select max(id) as id from bigdata where ifno='1845' and DATE(date)=current_date) k);update filebackup set check_value=1 where id = (select k.id from (select max(id) as id from filebackup where ifno='${status}' and DATE(date)=current_date) k);`,
    (err, res) => {
      if (err) {
        console.log(err);
        result(err);
