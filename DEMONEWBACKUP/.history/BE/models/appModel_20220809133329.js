...
Backup.changeStatus = (status, result) => {
  ...
  let mysqlstatus = `update mysql set check_value=1 where id = (select k.id from (select max(id) as id from mysql where ifno=${status} and DATE(date)=current_date) k)`;
  sql.query(bigdata, filebackup, mysqlstatus, (err, res) => {
    if (err) {
      console.log(err);
      result(err);
    } else {
