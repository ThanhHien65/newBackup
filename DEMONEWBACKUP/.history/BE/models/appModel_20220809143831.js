...
Backup.changeStatus = (status, result) => {
  let filebackup = `update filebackup set check_value=0 where id = (select k.id from (select max(id) as id from filebackup where ifno=${status} and DATE(date)=current_date) k);`;
  let bigdata = `update bigdata set check_value=0 where id = (select k.id from (select max(id) as id from bigdata where ifno=${status} and DATE(date)=current_date) k);`;
  let mysql = `update mysql set check_value=0 where id = (select k.id from (select max(id) as id from mysql where ifno=${status} and DATE(date)=current_date) k);`;
