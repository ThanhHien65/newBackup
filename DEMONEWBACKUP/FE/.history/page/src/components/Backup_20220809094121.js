...
const Backup = () => {
  ...
  const showStatus = (e) => {
    ...
    console.log(backupDay[e.target.className]);
    const ob = backupDay[e.target.className]
    const fill = backupDay[e.target.className].map(
      (item, index) => backupDay[e.target.className].item == 0
    );
    console.log(fill);
  };
  return (
