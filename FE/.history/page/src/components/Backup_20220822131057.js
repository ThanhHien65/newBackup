...
const Backup = () => {
  const dataBackupDaily = useRecoilValue(BackupDaily);
  const dataASPBackup = useRecoilValue(ASPBackup);
  const Backupdetail = useRecoilValue(Backupstatus);
  const [backupDay, setBackupDay] = useState(dataBackupDaily);
  const [diskASPBackup, setdiskASPBackup] = useState(dataASPBackup);
  ...
  const confirm = () => {
    ...
    const checked = (e) => {
      // const newBackup = { ...aBackup[0] };
      // Object.keys(newBackup).forEach((item, index) => {
      //   if (newBackup[item] == 0) {
      //     newBackup[item] = 1;
      //   }
      // });
      // aBackup.length = 0;
      // setAbackup([newBackup]);
      Object.keys(aBackup[0]).forEach((item, index) => {
        if (aBackup[0][item] == 0) {
          // aBackup[item] = 1;
          console.log(Backup[0].bvalue);
        }
      });
      setAbackup([...aBackup]);
      console.log(backupDay);
      console.log(aBackup);
    };
    return (
      <div
