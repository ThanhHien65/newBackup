...
const Header = () => {
  const ChangeColor = (e) => {
    ...
    // console.log(findURL);
    const itemm = document.getElementsByClassName("backup")[0];
    console.log(itemm);
  };
  window.onload = () => {
    const itemm = document.getElementsByClassName("backup")[0];const  = document.getElementsByClassName("backup")[0];
    // const findURL = document.URL.split("/").pop();
    // if (findURL == "monitor") {
    //   e.target.style.backgroundColor = "#CCCC";
    // } else if (findURL === "traffic") {
