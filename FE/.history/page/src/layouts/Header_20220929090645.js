...
const Header = () => {
  ...
  window.onload = () => {
    const item = document.getElementsByClassName("backup")[0];
    const item = document.getElementsByClassName("monitor")[0];
    const item = document.getElementsByClassName("traffic")[0];
    const findURL = document.URL.split("/").pop();
    if (findURL == "monitor") {
      e.target.style.backgroundColor = "#CCCC";
    } else if (findURL === "traffic") {
