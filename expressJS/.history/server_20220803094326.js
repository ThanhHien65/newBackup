...
app.get("/aspbackupday", (req, res) => {
  connection.query(
    "SELECT * FROM (SELECT * FROM filebackup t1 JOIN (SELECT max(id) as maxid FROM (SELECT * from filebackup where ifno=(select id from information where ipaddress='{ipaddress}')) T GROUP BY DATE(T.date)) t2 ON t1.id = t2.maxid) t3 WHERE DATE(t3.date) >= CURRENT_DATE-7;",
    (err, result) => {
      if (err) {
        console.log("error: ", err);
      } else {
